...
# Build-time LSan-off hook (§6.1, mayhem/lsan_off.c) — replaces a forbidden runtime ASan
# default-options override; ASan use-after-free/overflow + UBSan stay fully on/halting.
# (SPEC.md §6.2 item 15 bans the override symbol NAMES anywhere under mayhem/, comments included,
# so the forbidden construct is described in prose here rather than named.)
...
$CC $SANITIZER_FLAGS $DEBUG_FLAGS $SQLITE_DEFS -I"$BUILD" \
    "$SRC/test/ossfuzz.c" "$BUILD/sqlite3.o" "$BUILD/lsan_off.o" \
...
$CC $SANITIZER_FLAGS $DEBUG_FLAGS $SQLITE_DEFS -I"$BUILD" \
    "$SRC/test/ossfuzz.c" "$STANDALONE_FUZZ_MAIN" "$BUILD/sqlite3.o" "$BUILD/lsan_off.o" \
